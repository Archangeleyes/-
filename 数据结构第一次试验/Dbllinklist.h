...
//  抽象数据类型方法声明及重载编译系统默认方法声明:
	...
	void SetCurPosition(int position) const;							//    000设定当前位置
	void FstNode(const ElemType &e);				// 给线性表末尾增加一个节点
...
template <class ElemType>
void DblLinkList<ElemType>::FstNode(const ElemType &e)
{
	DblNode<ElemType> * pFst, *temPtr, *nextPtr;
	temPtr = GetElemPtr(Length());
	nextPtr = temPtr->next;
	pFst = new DblNode<ElemType>(e, temPtr, nextPtr);
	temPtr->next = pFst;
	nextPtr->back = pFst;
	curPtr = pFst;					// 设置指向当前位置的指针
	count++;						// 插入成功后元素个数加1 
	curPosition = count;				// 设置当前位置的序号
}
...
////新加的部分
template<class ElemType>
void DblLinkList<ElemType>::SetCurPosition(int position) const
{
	curPtr = GetElemPtr(position);
}
////新加的部分
template <class ElemType>
...
//	position合法时返回SUCCESS,否则返回RANGE_ERROR
{
	...
	{	// position合法
		...
		if (position == Length())
		{	// 删除尾结点,当前结点变为position-1节点
			curPosition = position - 1;					// 设置当前位置的序号
			curPtr = tmpPtr->back;						// 设置指向当前位置的指针
		}
		else
